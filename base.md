...
### 分页
32位系统, 地址用4B表示, 一级页表(PageTable)前20bit是页表项(Page Table Entry, PTE)的索引, 后12bit是页大小, 但实际使用中, 因为已约定页大小是4k, 后12bit实际是空闲的, 因此可用作页属性. 二级页表的每个页表的页大小是`1024*4K=4M`, 保存的是下级物理页的物理地址, 虚拟地址的前10bit定位二级页表, 中间10bit定位页表中的物理页, 后12表示4k中的偏移量.

页表项中的地址是物理地址.

线性地址->物理地址: 用线性地址的高 20 位在页表中索引页表项，用线性地址的低 12 位与页表项中的物理地址相加.
