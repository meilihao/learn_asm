...
> 通常约定: cx 寄存器用作循环的次数控制, bx 寄存器用于存储起始地址.
...
- ES (Extra Segment) : 附加段寄存器, 附加数据段的段基值，称为“附加”是因为此段寄存器用途不像其他 sreg 那样固定，可以额外做他用
    ...
    `mov [es:0],ax` 将ax赋值给地址`es+0`所指向的内存. 方括号表示取值
- SS (Stack Segment) : 堆栈段寄存器, 堆枝段的段值

数据寄存器可当做两个独立的8bit寄存器使用, 分别用${X}H/${X}L表示, 其他寄存器不可拆分.

## 32 bit
32 位 CPU 具有保护模式和实模式两种运行模式, 可以兼容实模式下的程序. 兼容实模式是指能够正确处理好实模式下的程序, 并不是说在实模式下运行时就完全变成了纯 16 位的 CPU.

当它以 16 位的实模式运行时, 不是说变成纯粹的 16 位的 CPU了（硬件不会变身）, 32 位 CPU 在 16 位的实模式下运行时, 可认为是更为强大的 16 位的 CPU, 但其本质可是 32 位的. 所以, 当它在 16 位的实模式中, 依然具备处理 32 位操作数的能力. 该能力会体现在反汇编出的指令的反转前缀上.
...
- EBP(Base Pointer) : 基指针寄存器
    ...
    在 32 位环境下， ebp 就应用在堆栈框架中, 堆栈框架是编译器在栈中为局部变量分配内存空间的方式.

变址寄存器
- ESI : (Source Index) : 源变地寄存器
- EDI : (Destination Index) : 目的变址寄存器

> 通俗约定: esi 寄存器作为很多有关数据复制指令的源地址, edi 作为目的地址.
...
地址是描述各种符号在源程序中的位置, 它是源代码文件中各符号偏移文件开头的距离. 当前地址等于上一个地址＋上一个地址处的内容的长度.

编译器的工作就是给各符号编址. 它给程序中各符号（变量名或函数名等）分配的地址, 就是各符号相对于文件开头的偏移量.
...
> cpu最早是没有实模式的, 有了保护模式后, 为了区分就将以前的模式称为实模式.

实模式和保护模式主要区别就是对内存地址的访问方式不同.

实模式被保护模式淘汰的原因, 最主要是安全隐患. 在实模式下, 用户程序和操作系统可以说是同一特权的程序, 因为实模式下没有特权级, 它处处和操作系统平起平坐, 所以可以执行一些具有破坏性的指令.
...
- 局部性段描述表寄存器LDTR（Local Descriptor Table Register ）, 现代处理器未使用ldtr.
...
- 15-3 : 2^13=8192, 又因为TI, 因此存在`8192*2=16384`个段描述符. 可从全局或局部描述符表项中选择一个描述符
    ...
    > cpu规定gdt中第一个段选择子不可用, 避免因忘记初始化选择子而选择到第 0 个段描述符的情况, 此时cpu会发出异常

    > 与 GDT 不同的是 LDT 中的第 0 个段描述符是可用的. 因为此时TI位为1, 必然是经过显式初始化的结果, 完全排除了忘记初始化的可能.
...
typedef struct
{
 ...
 unsigned  int  p:1;                /* segment present，为0时表示该段不在内存中, 由cpu检查*/
 unsigned  int  dpl:2;              /* Descriptor privilege level，访问本段所需的权限. 0, 最高, os用; 3, 最低, 用户程序用*/
 unsigned  int  s:1;                /* 描述项类型，0表示系统(硬件运行需要的东西)，1表示数据(软件运行需要的东西, 包括代码,数据,栈等) */
 unsigned  int  type:4;             /* 段的类型，与S标志位一起使用才有意义*/ [当S=0时，TYPE域规定](https://xem.github.io/minix86/manual/intel-x86-and-64-manual-vol3/o_fe12b1e2a880e0ce-102.html), [当S=1时，TYPE域规定](https://xem.github.io/minix86/manual/intel-x86-and-64-manual-vol3/o_fe12b1e2a880e0ce-100.html)
 unsigned  int  base_0_23:24;       /* 基地址的低24位 */
 unsigned  int  seg_limit_0_15:16;  /* 段长度的低16位 */
};
```

段最大大小=`seg_limit*g`=`2^20*1B/2^20*4K`=1M/4G. 段界限边界值＝（描述符中段界限seg_limit+1) * （段界限的粒度大小g： 4k/1B) -1, 根据段的扩展方向, 此段界限边界值便是段内偏移地址的最大值(向上扩展)或最小值(向下扩展), 任何超过此值的偏移地址都被认为是非法访问, CPU 会将此错误捕获.

> 段基址被拆成3段, 段界限被拆成2段是为了兼容性的历史遗留问题.

非系统段type:
- bit 3: 是否可执行. 1, 代码段Code; 0, 数据段Data
- bit 0~2:
    
    Code:
    - C: 一致位. 1, 一致性代码段; 0, 非一致性代码段
        ...
        定义这个概念主要是为了系统安全：内核要和用户程序分开，内核一定要安全不能被用户程序干涉. 但有时候用户程序也需要读取内核的某些数据. 于是操作系统内核程序开辟一些可以供用用户程序访问的段, 但是不允许用户程序写入数据. 内核不用知道用户程序的数据, 内核不用调用用户程序的数据, 内核不用转移到用户程序中来. 用户程序只能访问到内核的某些共享的段, 就称这些段为一致代码段, 用户程序不能访问内核不共享的段.
    - R: 是否可读. 不可读的代码段只是来限制代码指令的, 并不是连 CPU 也不能看
    - A: 访问位, 每当该段被 CPU 访问过后, CPU 就将此位置1. 创建一个新段描述符时, 应该将此位置0. 调试时, 根据此位便能判断该描述符是否已用

    Data:
    - E: 段的扩展方向. 0, 向上扩展, 地址越来越高, 常用于数据段; 1, 向下扩展, 地址越来越低, 通常用于栈段
    - W: 是否可写. 1, 可写; 0, 不可写
    - A: 访问位, 同Code的A

段描述符缓冲寄存器: 段描述符是在内存中, 不利于高速访问, 因此cpu有不可见的寄存器缓存它, 该寄存器就叫段描述符缓冲寄存器. 32位cpu中, 在实模式下时, 实际也利用了它, 段基址左移 4 位后的结果就被放入段描述符缓冲寄存器中, 以后每次引用一个段时, 就直接走段描述符缓冲寄存器, 直到该段寄存器被重新赋值.

cpu加载选择子时会检查用途和段类型是否匹配, 大的原则如下:
- 只有具备可执行属性的段（代码段〉才能加载到 cs 段寄存器中, 且不允许加载到除 cs 外的段寄存器中
- 只有具备可写属性的段（数据段）才能加载到 SS 栈段寄存器中;
- 至少具备可读属性的段才能加载到 DS 、 ES 、FS、 GS 段寄存器中

cpu 检查完 type 后, 还会再通过段描述符中的 P 位检查段是否存在, 如果存在, 则将选择子载入段寄存器了, 同时段描述符缓冲寄存器也会更新为选
择子对应的段描述符的内容，随后处理器将段描述符中的 A 位置为 1 ，表示己经访问过了; 如果 P 位为0, 表示该内存段不存在, 不存在的原因可能是由于内存不足, 这时候处理器会抛出异常, 自动转去执行相应的异常处理程序, 异常处理程序将段从硬盘加载到内存后并将 P 位(p位是os设置, cpu检查)置为 1, 随后返回. CPU 继续执行刚才的操作, 判断 P 位.
...
3. 将 crO 的 pe 位置 1
    ...
    CRO寄存器的第 0 位即 PE(Protection Enable) 位, 此位用于启用保护模式
...
### 16 bit/单片机寻址
**实模式下寻址的偏移量不能超过16位; 保护模式下偏移量由实模式的 16 位变成了 32 位**

- 立即数寻址: 操作数是常数, 而不是在寄存器或内存中. 比如`mov ax, 0x10`
- 寄存器寻址: 操作数在寄存器中, 直接从寄存器中拿数据就行, 比如`mov ds, ax`, `mul dx`

    指令中可以引用的寄存器及其符号名称如下：
    - 8位寄存器有：AH、AL、BH、BL、CH、CL、DH和DL等
    - 16位寄存器有：AX、BX、CX、DX、SI、DI、SP、BP和段寄存器等
    - 32位寄存器有：EAX、EBX、ECX、EDX、ESI、EDI、ESP和EBP等
- 内存寻址: 操作数在内存中, **默认数据段寄存器是ds**

    内存寻址细分:
    - 直接寻址方式: 直接在操作数中给出的数字作为内存地址, 比如`mov ax, [0x1234]`(=`mov ax, [ds:0x1234], addr=ds*16+0x1234`), `mov ax, [fs:0x1234]`
    - 基址寻址: 在操作数中有且只能用 bx/bp 作为地址的起始, 地址的变化以它为基础. 比如操作栈时的`[ebp-4]`

        bx 寄存器的默认段寄存器是 DS, 而 bp 寄存器的默认段寄存器是 SS.

        **保护模式下, 基址寄存器不再只是 bx/bp, 而是所有 32 位的通用寄存器, 变址寄存器也是一样，不再只是 si 、 di, 而是除 esp 之外的所有 32 位通用寄存器**.
    - 变址寻址: 与基址寻址类似, 有且只能用 si/di. 它主要是用于字符搬运方面的指令.
    ...
    - 基址加变址寻址/相对基址加变址寻址
        ...
        基址加变址寻址: 基址寄存器 bx 或 bp 加一个变址寄存器 si 或 di, 比如`mov [bx+di], ax`(即[ds:bx+di]=ax)
...
- physical address(物理地址): 出现在CPU外部地址总线上的寻址物理内存的地址信号, 由硬件处理
    ...
    linear address在分页机制下, 需经处理器分页映射转换为最终的物理地址. 地址转换是由处理器和操作系统共同协作完成的, 处理器在硬件上提供地址转换部件, 操作系统提供转换过程中所需要的页表.
...
> 平坦模型是相对于多段模型来说的, 即平坦模型指的就是一个段, 就是整个内存在一个段中, 即无论段选择子值是多少, 其段基址都是0.
...
### 分页
ref:
- [存储器管理——两级页表](https://www.cnblogs.com/LXZYZDCZL/articles/16307791.html)

32位系统, 页大小是4K, 页表项用4B表示, 4K/4B=1024=2^10, 即每个页可以放1024个页表项, 即各级页表最多1024个, 即需要逻辑地址中的10bit来映射页表.


一级页表(PageTable)前20bit是页表项(Page Table Entry, PTE)的索引, 后12bit是页大小, 但实际使用中, 因为已约定页大小是4k, 后12bit实际是空闲的, 因此可用作页属性.
二级页表的每个页表的页大小是`1024*4K=4M`, 保存的是下级物理页的物理地址, 虚拟地址的前10bit定位二级页表, 中间10bit定位页表中的物理页, 后12表示4k中的偏移量.
一级页表最多1M个页表项. 二级页表是将这 1M 个标准页平均放置 1K 个页表中, 每个页表中包含有 1K 个页表项.

一级页表时, 线性地址到物理地址的映射: 用线性地址的高 20 位在页表中索引页表项，用线性地址的低 12 位与页表项中的物理地址相加.

页表项中的地址是物理地址.

每个页表的物理地址在页目录中以页目录项(page directory entry, PDE)的方式存储.

页属性:
- P(Present, 存在位, 1bit): 1, 该页存在于物理内存中; 0, 该表不在物理内存中. 操作系统的页式虚拟内存管理便是通过 P 位和相应的 pagefault 异常来实现的
- RW(Read/Write, 读写位, 1bit) :1, 可读可写; 0, 可读不可写
- US(User/Supervisor, 普通用户/超级用户位) : 1, 处于 User 级, 任意级别（ 0 、 1 、 2 、3 ）特权的程序都可以访问该页; 0, 处于 Supervisor 级, 特权级别为 3 的程序不允许访问该页, 该页只允许特权级别为 0、 1 、 2 的程序可以访问
- PWT(Page-level Writ-through, 页级通写位也称页级写透位, 1bit): 1, 此项采用通写方式, 表示该页不仅是普通内存, 还是高速缓存. 此项和高速缓存有关，“通写”是高速缓存的一种工作方式, 本位用来间接决定是否用此方式改善该页的访问效率
- PCD(Page-level Cache Disable, 页级高速缓存禁止位, 1bit) : 1, 该页启用高速缓存; 0, 禁止将该页缓存
- A(Accessed, 访问位, 1bit): 1, 该页已被 CPU 访问过, 该位是由 CPU 设置的. 段描述符中的 A 位和 P 位在一起可以实现段式虚拟内存管理. 和它们一样, 页目录项和页表项中的 A 位也可以用来记录某一内存页的使用频率（操作系统定期将该位清 0, 统计一段时间内变成 1 的次数), 从而当内存不足时, 可以将使用频率较低的页面换出到外存（如硬盘）, 同时将页目录项或页表项的 P 位置 0, 下次访问该页引起 pagefault 异常时, 中断处理程序将硬盘上的页再次换入, 同时将 P 位置 1.
- D(Dirty, 脏页位, 1bit) : 当 CPU 对一个页面执行写操作时, 就会设置对应页表项的 D 位为 1. 此项仅针对页表项有效, 并不会修改页目录项中的 D 位.
- PAT (Page Attribute Table, 页属性表位, 1bit) : 能够在页面一级的粒度上设置内存属性, 比较复杂
- G(Global, 全局位, 1bit) : 由于内存地址转换也是颇费周折, 先得拆分虚拟地址, 然后又要查页目录, 又要查页表的, 所以为了提高获取物理地址的速度, 将虚拟地址与物理地址转换结果存储在 TLB (Translation Lookaside Buffer, 用来缓存地址转换结果的高速缓存）中. 此 G 位用来指定该页是否为全局页, 1 是全局页,  0 则不是全局页. 若为全局页, 该页将在 TLB 中一直保存, 给出虚拟地址直接就出物理地址, 无需那三步骤转换. 由于 TLB 容量比较小, 所以这里面就存放使用频率较高的页面. 清空 TLB 有两种方式, 一是用 invlpg指令针对单独虚拟地址条目清理, 或者是重新加载 cr3 寄存器, 这将直接清空 TLB.
- AVL(Available 位, 3bit) : 操作系统可用该位, CPU 不理会该位的值

启用分页机制, 需要按顺序做好三件事:
1. 准备好页目录表及页表
2. 将页表地址写入控制寄存器 cr3

    cr3用于存储页表的物理地址, 又称为页目录基址寄存器(Page Directorγ Base Register, PDBR), 只要在 cr3 寄存器的第 31 ～ 12 位中写入物理地址的高 20 位就行. 另外, cr3 寄存器的低 12 位中, 除第 3 位的 PWT 位和第 4 位的 PCD 位外, 其余位都没用.

    页目录表所在的地址要求在一个自然页内, 即页目录的起始地址是 4KB的整数倍. 
3. 寄存器 cr0 的 PG 位置 1

## 显示
显示器有多种模式, 比如图形模式, 文本模式等; 也支持多种显示方式, 比如80*25, 40*25等.

显卡加电后默认处于`文本模式下+80*25`, `80*25=2000`但一个字符需用2B属性(低1B是字符的ascii码, 高1B是字符属性), 因此`2000*2=4000<4K`, 一屏(4k)就是一页, 0页即默认页. 32k/4k=8, 因此linux支持alt+fn切换tty.

2B属性:
```c
typedef struct
{
 // 高字节中, 高4bit是字符背景色, 低4bit是字符前景色
 unsigned  int  k:1;                /*是否闪烁 */
 unsigned  int  r:1;
 unsigned  int  g:1;
 unsigned  int  b:1;                /*背景色rgb的b*/
 unsigned  int  I:1;                /* 亮度 */
 unsigned  int  r:1;
 unsigned  int  g:1;
 unsigned  int  b:1;                /*前景色rgb的b*/
 // 字符
 unsigned  int  c:8;  /* 字符的ascii码 */
};
```

在实模式下可以用 BIOS 的 0x1O 中断打印字符串, 这是因为：首先中断向量表只在实模式下存在, BIOS中断是要依赖于中断向量表的. 保护模式下就没有中断向量表, 所以也就无法用 BIOS 中断.
...
随着不断添加功能, 内核必然越来越大, 其所在的内存地址也会向越来越高的地方发展, 难免会超过可用区域的上限, 应尽量把 loader 放在低处, 多留出一些空间给内核.