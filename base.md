...
段寄存器(segment register,sreg)
- CS (Code Segment) : 代码段寄存器, 代码段的段基值
- DS (Data Segment) : 数据段寄存器, 数据段的段基值
- ES (Extra Segment) : 附加段寄存器, 附加数据段的段基值，称为“附加”是因为此段寄存器用途不像其他 sreg 那样固定，可以额外做他用
- SS (Stack Segment) : 堆栈段寄存器, 堆枝段的段值
...
- DS (Data Segment) : 数据段寄存器, 数据段的段基值
- ES (Extra Segment) : 附加段寄存器, 附加数据段的段基值，称为“附加”是因为此段寄存器用途不像其他 sreg 那样固定，可以额外做他用
- FS (Extra Segment) : 附加段寄存器, 同上
- GS (Extra Segment) : 附加段寄存器, 附加数据段的段基值
- SS (Stack Segment) : 堆栈段寄存器, 堆枝段的段值

在实模式下, CS 、 DS 、 ES 、 SS 中的值为段基址，是具体的物理地址，内存单元的逻辑地址仍为“段基值：段内偏移量”的形式; 在保护模式下，装入段寄存器的不再是段地址，而是“段选择子”(Selector). 在 32 位 CPU 中, sreg 无论是工作在 16 位的实模式, 还是 32 位的保护模式都是16位宽(2B).
...
见的地址, 是由程序产生的与段相关的偏移地址部分.

    `mov 0x80495b0, %eax`中的内存地址0x80495b0 就是一个逻辑地址, 必须加上隐含的DS 数据段的基地址, 才能构成线性地址. 因此0x80495b0 是当前任务的DS数据段内的偏移.
    
    最终的地址是由段基址和段内偏移地址组合而成的. 由于段基址已经有默认的值, 要么是在实模式下的默认段寄存器中, 要么是在保护模式下的默认段选择子寄存器指向的段描述符中, 所以只要给出段内偏移地址就行了. 即实模式下由`段基地址<<4+段内偏移`组成;保护模式下由`段选择符+段内偏移`组成.

    > Linux中逻辑地址等于线性地址. 因为Linux所有的段（用户代码段、用户数据段、内核代码段、内核数据段）的线性地址都是从 0x00000000 开始, 此时线性地址=逻辑地址+ 0x00000000, 即逻辑地址等于线性地址.
- linear address(线性地址): 逻辑地址到物理地址变换之间的中间层, 逻辑地址加上相应段的基地址就是一个线性地址. 它是cpu通过分段（Segment）机制后形成的地址空间. 线性地址在实模式和非分页的保护模式下就是物理地址.
...
- physical address(物理地址): 出现在CPU外部地址总线上的寻址物理内存的地址信号, 由硬件处理
...
如果从系统中任意一个processor的角度来看，当它访问物理内存的时候，物理地址空间是一个连续的，没有空洞的地址空间，那么这种计算机系统的内存模型就是Flat memory. 平坦模型一定在分页机制下运行, 分页机制是内存平坦模型的基础.

> 平坦模型是相对于多段模型来说的, 即平坦模型指的就是一个段.

如果cpu在访问物理内存的时候，其地址空间有一些空洞，是不连续的，那么这种计算机系统的内存模型就是Discontiguous memory。一般而言，NUMA架构的计算机系统的memory model都是选择Discontiguous Memory. 非连续模型只是单纯对这块内存进行标记，并不进行维护，当热插拔出现之后，会使得原本连续的内存块变得不连续。而稀疏模型的块管理结构，就是可以对热插拔引起空洞，进行维护.

sparse memory最终可以替代Discontiguous memory.
