...
in 用于从端口中读取数据, 源操作数（端口号）必须是dx, 目的操作数是用al, 还是ax, 取决于 dx 端口指代的寄存器是 8还16 位宽度.

out 指令用于往端口中写数据, 其目的操作数是端口号, 其一般形式是：
1. out dx, al
1. out dx, ax
1. out 立即数, al
1. out 立即数, ax

in/out中, dx 只做端口号之用, 无论其是源操作数或目的操作数.
...
3. 往 device 寄存器中写入 LBA 地址的 24～27 位，并置第 6 位为 l ，使其为 LBA 模式，设置第 4位，选择操作的硬盘（ master 硬盘或 slave 硬盘）
4. 往该通道上的 command 寄存器写入操作命令
5. 读取该通道上的 sta阳s 寄存器，判断硬盘工作是否完成
